...
## 2026-08-22 — 사람이 손으로 잇는 단계가 남아 있으면 "자동화됐다"가 아니다

- 실패 모드: clone 스킬의 모든 개별 단계가 스크립트로 자동화돼 있었는데도 한방 실행이 불가능했다. `explore` 가 한 화면만 소진하고 멈춰 화면 사이 이동을 사람이 했고, 문서상 사람 게이트 4곳이 실행을 막았고, 검증은 화면마다 `<NN>` 을 갈아 끼우는 수동 반복이었다.
- 검출 신호: 스킬 문서에서 "사용자와 함께 고른다", "여기서 정한다", "검토·수정한다", "다시 실행한다"를 세어 보면 나온다. 개별 스크립트의 테스트는 전부 green 이었다 — 자동화의 구멍은 스크립트 안이 아니라 **스크립트 사이**에 있었다.
- 예방 규칙: 파이프라인의 완성도는 "단계마다 명령이 있는가"가 아니라 "사람이 개입해야 하는 지점이 몇 개인가"로 센다. 사람 게이트는 삭제가 아니라 **자율 기본값 + opt-in 플래그**로 바꾼다 — 삭제하면 다음 사람이 왜 없어졌는지 모른다. 화면·항목마다 반복하는 명령은 반드시 배치 진입점을 둔다. 자리표시자(`<NN>`)가 문서에 남아 있으면 그 단계는 아직 자동화되지 않은 것이다.

## 2026-08-22 — 두 번째 소비자를 추가할 때 producer 필드명을 다시 확인한다

- 실패 모드: `clone_flow_codegen.py` 가 `state`/`from_state`/`to_state` 만 읽는데 producer 는 flow v2 로 `statekey`/`from_statekey`/`to_statekey` 를 쓴다. 크래시 없이 coarse `node`/`from`/`to` 로 조용히 폴백해, 한 화면의 두 상호작용 상태(포커스된 검색창과 아닌 것)가 한 뷰로 뭉개졌다.
- 검출 신호: 테스트 이름이 `test_manifest_supports_new_and_old_state_fields` 인데 픽스처의 "new" 가 `state` 였다 — 작성자가 계약을 픽스처로만 알고 있었다는 신호. `device_wda.sh` 의 `_flow_event` 호출 한 줄만 보면 즉시 드러난다.
- 예방 규칙: 2026-08-16 lesson 의 교차 테스트는 **소비자마다** 필요하다. 같은 파일의 새 소비자를 추가할 때 기존 소비자가 green 이라는 사실은 아무 보증도 아니다. 새 소비자는 producer 의 실제 산출물을 그대로 통과시키는 테스트를 자기 몫으로 하나 갖는다.
- 2026-08-22: bash 3.2(macOS 기본)는 `$( ... )` 안의 `<<'HEREDOC'` 본문도 따옴표를 스캔한다. 파이썬 주석에 아포스트로피 하나를 넣자(`the run's own log`) 100줄 아래에서 `unexpected EOF while looking for matching`가 났다. 명령 치환 안의 heredoc 본문에는 홀수 개의 `'`를 두지 않는다 — 서술을 바꾸거나 heredoc을 치환 밖으로 뺀다.
- 2026-08-22: 같은 셸에서 `set -u`와 빈 배열 확장(`"${arr[@]}"`)은 bash 3.2에서 unbound variable이다. 선택적 플래그는 배열 대신 분기로 넘긴다.

## 2026-08-22 — 화면 정체성을 "라벨 집합"으로 잡으면 라이브 앱에서 탐험이 0 step 으로 죽는다

**실패 모드.** `_prepare_tap` 의 freshness 가드가 `sig`(라벨 집합의 해시)로 "캡처 이후
화면이 움직였는가"를 판정했다. Threads 실기기 실행에서 탐험이 **매번 0 step** 으로 끝났다.
원인은 좋아요 수가 실시간으로 올라가는 것 — 캡처와 탭 사이에 `좋아요 226명` → `227명`.
화면은 그대로인데 라벨 집합이 달라져 43개 후보가 전부 거절됐다. 라이브 피드에서 이 가드는
**통과가 구조적으로 불가능**하다.

**탐지 신호.** "explore stopped after 0 step(s)" + `planned <hash>, live <hash>`.
0 step 은 flaky 가 아니라 가드가 항상 참인 조건을 검사한다는 뜻이다.

**두 번째 겹.** 가드 거절이 `return 1` 이라 explore 전체가 중단됐다. 실기기에서 화면이
움직이는 것은 정상이고, 그때 할 일은 중단이 아니라 **재캡처**다.

**예방 규칙.**
1. 정체성을 고를 때 "무엇이 바뀌면 안 되는가"가 아니라 **"무엇이 반드시 바뀌는가"**를 먼저 본다.
   콘텐츠(카운터·타임스탬프·읽지 않음 배지)는 반드시 바뀐다. 화면 정체성에 콘텐츠를 넣지 않는다
   — 이 레포에는 이미 그 목적의 `nodekey`/`statekey` 가 있었는데 가드만 `sig` 를 썼다.
2. 가드는 **자신이 실제로 지켜야 하는 것**만 검사한다. 여기서 지킬 것은 "화면 전체가 동일한가"가
   아니라 "이 좌표가 여전히 같은 화면의 유효한 후보인가" 였다. 그래서 provenance 검사를
   **낡은 캡처가 아니라 라이브 트리**에 돌린다 — 더 느슨한 게 아니라 더 엄격하다
   (withheld 판정도 지금 화면 기준이 된다).
3. 탭 **전** 실패와 탭 **후** 실패를 다른 exit code 로 구분한다. 전자만 재시도해도 안전하고,
   후자를 재시도하면 실기기를 두 번 누른다.
4. 문서의 단정("sig 는 탭 가드에 맞다")을 측정이 반증하면 **문서도 같은 커밋에서 고친다**.
   안 고치면 다음 사람이 코드보다 산문을 믿는다.

**증거로 삼을 것.** 실기기 없이 통과하는 테스트 47개가 이 실패를 하나도 못 잡았다.
producer 가 만드는 **실제 변동**(콘텐츠 churn)을 픽스처에 넣어야 잡힌다 —
`tests/test_device_wda.py::TestLiveContentDoesNotBlockTaps`.

## 2026-08-22 — 검사기가 조용히 아무것도 검사하지 않는 세 가지 방법

Threads 실기기 회차에서 `verify` 를 처음 끝까지 돌렸더니, 검사기 셋이 각각 다른 방식으로
**검사하지 않으면서 검사한 것처럼** 보고하고 있었다.

1. **키 이름 drift** — `axe describe-ui` 는 `AXLabel` 을 쓰는데 `clone_structural_diff.py` 는
   `label` 을 읽었다. 라벨 매칭이 항상 빈 결과라 모든 요소가 프레임 매칭으로 떨어졌고,
   safe-area 만큼 밀린 요소가 전부 `missing` 이 됐다. WARN 경로는 죽은 코드였다.
   테스트 픽스처가 `label` 키를 **지어내** 통과하고 있었다 — 2026-08-16 lesson 과 같은 계열.
2. **degrade 를 pass 로 셈** — 첫 렌더가 시뮬레이터 부팅과 경합해 AXe 가 빈 결과를 내면
   구조 diff 를 건너뛰고 WARN 만 남겼다. 그 화면은 실패도 통과도 아닌 **미검사**인데
   실패 카운터가 오르지 않아 통과로 보고됐다.
3. **비교 자체를 거부** — 실기기 1124x2436 과 시뮬레이터 1080x2340 은 같은 375x812 pt 인데
   픽셀 크기가 다르다는 이유로 정량 지표 전부가 skip 됐다(60/60). "skip" 이 로그에 INFO 로
   나가 아무도 세지 않았다.

**예방 규칙.**
- 검사기의 테스트는 **producer 가 실제로 내보낸 산출물**로 한다. 픽스처를 손으로 지으면
  키 이름·형태가 어긋난 것을 절대 못 잡는다. 최소한 픽스처를 실제 출력 모양으로 베낀다.
- 검사가 **수행되지 않은 경우**는 통과가 아니다. 세 갈래(통과/실패/미검사)를 두 갈래로
  뭉개면 미검사는 항상 통과 쪽에 붙는다.
- `INFO: ... skipped` 가 반복되면 그건 정보가 아니라 **기능이 꺼져 있다는 신호**다.
  몇 번 skip 됐는지 세어 보면 바로 드러난다.
- 환경 실패(시뮬레이터 없음)를 항목마다 반복 보고하지 않는다. 원인을 한 번, 원인 자리에서
  말한다. 20번 반복되는 "fix the compiler diagnostics" 는 있지도 않은 원인을 지목했다.


## 2026-08-22 — 대상 앱을 벗어나면 다른 앱의 화면이 산출물이 된다

**실패 모드.** Threads 탐험이 `Instagram으로 전환` 을 탭했다. Appium `/source` 는 세션의
bundle 이 아니라 **지금 전면인 앱**을 기술하므로, Instagram 화면이 Threads 의 state 로
flow 로그에 기록됐다. 그대로 두면 측정·스펙·`views.json` 까지 그대로 흘러가
"복제 대상 앱의 화면" 으로 승격된다. 그 다음 탭에서 가드가 알아채고 실행 전체를 끝냈다.

**예방 규칙.**
- 액션 **전** 에만 컨텍스트를 확인하면 늦다. 화면을 증거로 저장하기 **직전**에도 확인한다.
  액션이 컨텍스트를 바꾸는 것이 바로 그 액션의 효과이기 때문이다.
- 컨텍스트를 벗어난 것은 **복구 가능**하다(대상 앱을 다시 올린다). 관측 도구가
  외부 요인으로 끝나면 그때까지의 실기기 세션이 통째로 낭비된다.
- 이탈을 기록할 때 **목적지 식별자를 그대로 쓰지 않는다.** 외부 앱의 state 를 남기면
  그래프가 증거 없는 노드로 라우팅하고, 커버리지 검사가 절대 만들어지지 않을 증거를
  요구한다 — 닫을 수 없는 공백. 종착 엣지(`changed=false`, 목적지 미해결)로 남긴다.


## 2026-08-22 — 안전 가드를 이상화된 라벨로 시험하면, 가드가 없는 것과 같다

**실패 모드.** 블라인드 탐험이 절대 누르면 안 되는 것(좋아요·리포스트·공유)을 실제 계정으로
눌렀다. `STATE_CHANGING` 패턴이 전부 라벨 **끝**에 앵커돼 있었는데, 실제 기기가 내보내는
라벨은 `좋아요. 226명이 이 게시물을 좋아합니다.` — **액션 이름 + 설명문**이다.
테스트는 `좋아요`, `Like post` 같은 짧은 라벨만 검사했다. 그런 라벨을 내보내는 앱은 없다.

**왜 놓쳤나.** 앵커는 의도가 옳았다 — `팔로우 추천`(화면)이 `팔로우`(액션)로 읽히면 안 된다.
정밀도를 **끝 앵커**로 구현한 것이 실패 지점이고, 그 선택이 실제 라벨 형태와 충돌한다는 사실을
픽스처가 가렸다.

**예방 규칙.**
- **안전 가드의 테스트 입력은 반드시 실기기에서 관측한 문자열이어야 한다.** 손으로 지어낸
  이상적 라벨로 통과하는 가드는 통과 사실 자체가 무의미하다. 이 레포에서 같은 계열의
  픽스처 실패가 이번 회차에만 두 번 나왔다(`AXLabel` vs `label`, 그리고 이 건).
- 가드의 **실패 방향**을 비대칭으로 잡는다. 과탐지는 탐험 한 칸을 잃고, 미탐지는 사용자
  계정을 바꾼다. 애매하면 withheld 쪽으로 기운다.
- 분류가 넓어졌는지 **세어서** 확인한다. 같은 로그에서 withheld 14 → 66 이 이 수정의 증거다.
  "패턴을 고쳤다"는 증거가 아니다.
- 실기기에서 파괴적일 수 있는 자동화를 돌릴 때는, 가드를 실제 라벨로 먼저 검증하고
  실행 중에도 무엇이 눌렸는지 감사한다. 이번엔 로그 감사로 3건을 찾아 사용자에게 알렸다.

## 2026-08-23 — 화면마다 다시 컴파일하고 있었다 (캐시 키가 루트 뷰를 담았다)

**실패 모드.** `device_render.sh` 의 렌더 캐시 키에 `root=$view` 가 들어 있었다. swiftc 는
어차피 `Sources/` 전체를 매번 컴파일하는데, 루트 이름이 키에 있으니 **화면 수만큼 같은 파일을
다시 컴파일**했다. 31개 화면이면 31회. 그리고 뷰를 하나 고치면 31개 캐시가 전부 무효가 된다 —
고치고 다시 재는 루프의 비용이 화면 수에 비례해 붙었다.

**예방 규칙.** 캐시 키는 **산출물이 실제로 의존하는 것**만 담는다. 루트 선택은 컴파일 산출물의
성질이 아니라 실행의 성질이므로 실행 시점으로 옮긴다(`CLONE_ROOT_VIEW` 환경변수 + 진입점의
switch). "무엇이 이 산출물을 바꾸는가"를 물으면 나오고, "무엇이 이 호출을 구분하는가"를 물으면
안 나온다. 두 질문은 다르다.

**부수 발견.** 화면마다 재설치하던 것도 같은 이유였다 — 이제 빌드가 바뀔 때만 설치하고 화면
전환은 재실행이다. 대신 **재실행은 이전 화면이 그대로 떠 있는 시간을 만든다**: "연속 2프레임
동일"이 새 화면이 아니라 **이전 화면**으로 만족될 수 있다. 실행 직전 프레임을 baseline 으로
잡아 그것과 같은 정지 프레임은 채택하지 않는다(끝내 그것뿐이면 실패가 아니라 WARN — 재현본이
정말 그렇게 보일 수도 있다).

## 2026-08-23 — 자리표시자에서 시작한 재현은 검증기를 무의미하게 만든다

**실패 모드.** Step 5 가 화면마다 빈 자리표시자로 시작했다. `verify` 는 31개 화면 전부에 대해
같은 말("요소 전부 누락")을 했고, 손으로 쓴 유일한 화면조차 163개 중 142개 누락이었다.
사람이 30개 화면을 손으로 써서 8pt 허용치에 맞추는 루프는 아무도 끝내지 못한다.

**측정으로 닫을 수 있는 실패였다.** 지배적 실패는 요소 누락(DCGen 85.3%)이고, 우리는 이미
모든 요소의 프레임·색·글자 크기를 측정해 갖고 있었다. `clone_view_codegen.py` 로 측정 하나당
노드 하나를 생성하니 27/28 화면이 **누락 0** 으로 통과했다.

**예방 규칙.** 검증기가 모든 항목에 같은 실패를 보고하면, 그건 항목의 문제가 아니라 **시작점의
문제**다. 사람이 손으로 채워야 하는 초기 상태를 두기 전에, 이미 가진 측정으로 기계가 채울 수
있는지 먼저 묻는다. 사람의 몫은 빈 화면을 채우는 것이 아니라 기계 초안을 의미 있는 구조로
옮기는 것이다 — 소유권 경계는 생성 마커 한 줄로 표시한다.

## 2026-08-23 — 세 번째 producer/reader drift: 새 소비자에 필드명을 지어냈다

**실패 모드.** `clone_view_codegen.py` 에서 전이를 뽑으며 `event.get("type") == "action"` 을
읽었다. producer 가 쓰는 것은 `"tap"`/`"swipe"` 다. 픽스처도 `"action"` 으로 지어 썼으므로
테스트 11개가 전부 green 이었고, 실기기 로그에서는 탭이 **하나도** 배선되지 않았다.

**예방 규칙.** 같은 파일의 새 소비자는 계약을 다시 구현하지 말고 **producer 모듈의 추출 함수를
그대로 임포트**한다. 여기서는 `clone_flow_codegen.observed_transitions` — ObservedFlow.swift 를
만드는 바로 그 함수다. 그러면 "라우터가 반응하는 키"와 "뷰가 탭으로 노출하는 키"가 같은 코드에서
나오므로 어긋날 수 없다. 이 레포에서 같은 계열 실패가 이번이 세 번째다(2026-08-16, 2026-08-22, 오늘).

## 2026-08-23 — 기능이 픽셀보다 먼저다 (사용자 교정)

**교정.** "처음부터 화면 하나하나를 완벽하게 깎아내는 것보단 화면에서 기능적인 부분을 우선
처리한 후, 픽셀까지 따라서 폴리싱하는 건 앱이 정상동작한다는 걸 확인 후에."

**왜 맞는가.** 도달할 수 없는 화면의 간격을 맞추는 것은 그 값이 셈에 들어가는지 모르는 채
쓰는 시간이다. 그리고 기능 검사는 픽셀 검사가 절대 못 보는 것을 본다 — 실제로 이 회차에서
`allowsHitTesting` 누락 때문에 **탭 4건 중 3건이 아무 데도 가지 않았고**, 구조 diff 와 픽셀
지표는 그 화면들을 전부 통과시켰다. 정지 화면만 보는 검사기는 정지 화면만 검증한다.

**적용.** `clone_run.sh` 를 `functional`(빌드 → 띄우기 → 관측 전이를 앱 안에서 탭 → 도착
화면 확인) 과 `polish`(렌더 → 구조 diff → 대조) 로 나눴다. `verify` 는 전자가 실패하면 후자로
넘어가지 않는다.

## 2026-08-23 — 측정 그대로 재생한 컨트롤은 탭할 수 없다 (원본이 44pt 를 안 지켰다)

**실패 모드.** 재현본의 전이 중 두 개가 끝까지 살아나지 않았다. 접근성 트리는 컨트롤을
**정확한 프레임으로 보고**했고, 드라이버는 그 중심에 탭을 성공적으로 보냈고, 로그에도 아무
오류가 없었다. 그런데 상태는 바뀌지 않았다.

**진단이 오래 걸린 이유.** 신호가 전부 "정상"이었다. 겹침·z-order·`allowsHitTesting`·
`.offset` vs `.position`·라벨 인코딩을 차례로 배제하는 데 대부분의 시간이 갔다. 결정적 데이터는
**성공한 것과 실패한 것의 크기를 나란히 적었을 때** 나왔다 — 성공 8건은 전부 23pt 이상,
실패 2건은 18×18 과 35×**3** 이었다.

**진짜 원인.** 원본 앱의 컨트롤이 44pt 터치 타깃 규칙을 지키지 않는다(3pt 짜리 시트 손잡이,
18pt 오버플로 점). 그 프레임을 **그대로** 재생하면 재현본도 같은 크기가 되고, 시뮬레이터에
전달된 탭은 그 안에 들어가지 않는다. 28pt 로 키운 것으로는 부족했고 **44pt(HIG 최소)** 에서
살아났다.

**예방 규칙.** 측정을 재생할 때 **보이는 것과 만지는 것을 분리한다.** 그림은 측정 그대로,
탭 타깃은 측정 중심을 유지한 채 44pt 로 키운 별도의 투명 레이어. 원본의 결함(접근성 미준수)까지
복제할 이유가 없고, 복제하면 재현본은 조작 자체가 불가능해진다.

**부수 규칙.** 원인을 못 찾겠으면 **성공 사례와 실패 사례의 속성을 표로 적는다.** 가설을 하나씩
반증하는 것보다 빠르다 — 여기서는 표를 적자마자 크기 축이 보였다.

## 2026-08-23 — 개선을 주장하기 전에 그 이득이 내 코드에서 나온 것인지 가른다

**실패 모드.** 폴리싱 1차에서 두 가지를 함께 넣고 "평균 mismatch 20.5% → 19.2%" 로 보고하려
했다. 두 변화 중 하나는 **원본 스크린샷에서 잘라낸 crop 을 재현본에 붙이는 것**이었다.
`device_compare` 는 재현본을 바로 그 `raw/<stem>.png` 와 대조한다 — crop 영역은 X 를 X 와
비교하므로 mismatch 가 구조적으로 0 이다. 그 영역이 화면 픽셀의 **18~39%** 를 덮는 화면이
여럿이었다.

**가른 방법.** `CLONE_ASSET_DIR` 을 없는 경로로 두고 같은 코드로 한 번 더 돌려 세 갈래를
만들었다 — 재생만 / +라벨 / +crop. 결과: 라벨 **−0.08pt**(이득 없음), crop **+1.44pt**.
개선분은 **전부** 붙여넣기 쪽이었고, 내가 쓴 규칙은 아무 것도 하지 않았다.

**부수 결론.** 그 라벨 규칙은 순효과 0 이면서 실제 결함을 만들었다(아이콘 위 접근성 문장 23건).
막으려면 휴리스틱 세 개가 더 필요했고, 규칙이 더해 주는 듯 보였던 문구는 **전부 같은 화면의
측정된 `AXStaticText` 로 이미 그려지고 있었다.** 확인하고 삭제했다.

**예방 규칙.**
1. 지표를 좋게 만드는 변경이 **지표의 기준(reference)에서 온 데이터**를 쓰면, 그 지표는 그
   변경을 평가할 수 없다. 넣기 전에 "이 값이 정답지에서 나온 것인가"를 묻는다.
2. 두 가지를 같은 회차에 넣었으면 **하나를 끄고 한 번 더 잰다.** 껐다 켜는 스위치가 없으면
   그것부터 만든다(여기서는 env 하나로 충분했다).
3. 이득이 0 인 규칙은 남기지 않는다. 산문으로 정당화하고 싶어지면 그게 신호다.

## 2026-08-23 — 대조 이미지를 눈으로 한 번 보는 것이 지표 열 번보다 빨랐다

**상황.** 픽셀 mismatch 를 여러 회차 재면서 20% 언저리에서 움직이지 않았다. 원인을 지표로
좁히려 했지만 region advisory 는 "어느 role 이 나쁘다"까지만 말했다.

**대조 이미지를 열어 보니 즉시 보였다.** 하트·댓글·리포스트·공유 아이콘, 상단바, 탭바가
**통째로 비어 있었다.** 아이콘은 접근성 트리에 그림이 아니라 설명(`좋아요. 226명이…`)으로만
존재하므로 어떤 측정으로도 재현할 수 없다 — 이 사실은 숫자에는 절대 나타나지 않는다.

**예방 규칙.** 지표가 정체되면 **산출물을 직접 본다.** 이 스킬의 기준은 애초에
"나란히 놓으면 같은 화면"이고, 그 판정은 사람 눈이 한다. 숫자는 회귀를 막는 도구이지
무엇이 틀렸는지 알려주는 도구가 아니다.

## 2026-08-23 — 픽셀이 조용한 것과 앱이 뜬 것은 다르다

**실패 모드.** 화면 5개가 "요소 전부 누락"으로 기록됐다. 접근성 트리가 통째로 비어 있었다 —
연속 두 프레임이 같아서 캡처를 확정했는데, 그 두 프레임은 **앱이 아직 안 뜬 홈 화면**이었다.
홈 화면도 조용하다.

**예방 규칙.** "변화가 멈췄다"를 준비 완료로 쓰지 않는다. 준비의 증거는 **무엇이 화면에
있는가**다 — 여기서는 접근성 트리가 이 앱 이름을 말하는 것. 이미 그 트리를 뽑고 있었으므로
검사에 추가 비용도 없었다.

**부수 교훈.** 처음엔 실패할 때마다 다시 캡처하게 만들었다가 처리량이 반으로 떨어졌다.
아직 안 뜬 것이 원인이면 할 일은 **기다리는 것**이고, 다시 찍는 것은 준비된 뒤 한 번이다.

## 2026-08-23 — 컨테이너를 "픽셀을 덮은 것"으로 세면 장식 이미지가 영원히 안 보인다

**실패 모드.** Threads 게시물 아바타 5개가 재현본에서 통째로 빠졌다 — 게시물마다 왼쪽 열이
빈 공간이었다. 아바타는 **접근성 트리에 아예 없다**(장식 요소). 그런데 바로 그것을 잡으라고
만든 `uncoveredRegions` 스캔은 **0건**을 보고했다.

**원인.** 스캔이 "어떤 측정 프레임이 이 픽셀을 덮는가"를 물으면서 **컨테이너까지 셌다.**
피드의 행 컨테이너가 아바타 자리를 덮고 있으니 "설명됨"으로 처리됐다. 컨테이너는 자식의
영역을 덮지만 거기에 아무것도 그리지 않는다.

**예방 규칙.** "이 픽셀은 무엇이 설명하는가"의 답은 **잎 요소**뿐이다. 커버리지 계산에
컨테이너를 넣으면 스캔은 항상 조용하고, 조용한 검사기는 없는 검사기다.
(같은 계열: 2026-08-22 "검사기가 조용히 아무것도 검사하지 않는 세 가지 방법".)

**부수 규칙.** 접근성 트리에 없는 픽셀은 어떤 측정으로도 재현할 수 없다. 그것을 재현하려면
캡처 crop 밖에 없고, 그러면 지표에서 그 영역을 빼고 재야 한다 — `--mask-assets` 가 이미
그렇게 한다. 재현할 수 없는 것을 재현한 척하지 않는 방법은 **세어서 말하는 것**이다.

## 2026-08-23 — 출발선만 재는 게이트는 실행이 그 공간을 다 쓰는 것을 못 막는다

**실패 모드.** `doctor` 의 디스크 게이트가 `free 3538MB (minimum 2048MB)` 로 통과했다. 그리고
같은 실행이 WDA/Xcode 빌드·캡처·렌더 캐시로 그 3.5GB 를 전부 쓰고 **ENOSPC 로 죽었다.**
게이트는 거짓말을 하지 않았다 — 시작 시점에는 정말 충분했다.

**예방 규칙.** 자원 게이트의 임계값은 "시작할 수 있는가"가 아니라 **"이 실행이 얼마나 쓰는가"**로
정한다. 그 값은 재 봐야 안다(여기서는 3.5GB). 재기 전까지의 임계값은 추측이고, 추측한 게이트는
통과시켜 놓고 나중에 죽게 만든다.

**부수 관찰.** 실패는 clone 과 무관한 곳에서도 났다 — 같은 디스크에 8일 된 Xcode DerivedData
사본 두 개가 3.8GB 를 잡고 있었다. 정리 대상을 고를 때 **내 것 / 재생성 가능한 것 / 사용자
것**을 구분하고, 마지막은 묻는다.

## 2026-08-23 — "되다 만 앱"의 원인은 재현이 아니라 관측이었다 (사용자 리뷰)

**교정.** "스크롤도 안 되고, 버튼을 클릭해도 아무런 액션이 없고, 한 마디로 앱 복제가 되다 만 듯해."

**진단.** 두 원인이 겹쳤다.
1. 탐험이 깊은 화면에서 포기했다 — `next-tap` 은 관측된 전이로만 길을 찾아, 설정 깊은 곳에서
   남은 37개 화면으로 갈 길이 없으면 거기서 끝났다. 타깃 522개 중 100개. **관측이 없는 버튼은
   배선도 없다.** 재현본의 죽은 버튼은 재현의 결함이 아니라 관측의 구멍이었다.
2. ScrollView 가 아예 없었다 — 109/122 캡처에 스크롤 컨테이너가 있는데 고정 스택으로 재생했다.

**예방 규칙.**
- 탐험의 종료 조건은 "여기서 할 게 없다"가 아니라 "**앱 어디에도** 할 게 없다"여야 한다.
  막히면 되돌아가는 수단(재시작)을 탐험기 자체가 가져야 한다 — 사람이 손으로 이동시켜 주는
  설계는 이미 한 번 걷어냈는데(8/22) "경로 없음"이 같은 구멍으로 되돌아왔다.
- 탭이 전부가 아니다. 텍스트 필드는 **키보드 너머에만 있는 화면**으로 가는 유일한 문이다.
- 재현본을 **실기기에 올려 손으로 써 본 것**이 이 회차의 결정적 검증이었다. 기능 게이트(72/73
  배선)도 구조 diff(47/47)도 픽셀 지표도 전부 통과한 상태에서 사용자가 30초 만에 본 것이다.
  정지 화면 검사기는 정지 화면만 검증한다 — 이 교훈은 이제 세 번째다.


## 2026-08-23 — 아래 형제의 제스처는 위 레이어의 탭을 가로챈다 (ScrollView 도, DragGesture 도)

**실패 모드.** 스크롤을 넣자 직전까지 멀쩡하던 탭바 탭이 죽었다. 기능 게이트는 73/73 → 25/92 로
주저앉았고, 원인을 찾는 데 프로브 6종이 들어갔다.

**원인 두 겹.**
1. iOS 26 시뮬레이터의 `ScrollView` 는 ZStack 에서 자기 위에 그려진 형제의 **가장자리** 탭을 삼켰다
   (중간은 되고 상·하단만 죽음). `.zIndex`, `.overlay`, `Button`, `highPriorityGesture`,
   `.ignoresSafeArea`, `.scrollEdgeEffectHidden` 전부 무효.
2. 그래서 드래그 오프셋으로 바꿨더니 **그 DragGesture 가 붙은 전체 화면 형제**가 또 위 레이어의 탭을
   가로챘다. 제스처만 떼면 탭바가 돌아왔다.

**해결.** 제스처를 **화면 루트(모든 타깃의 조상)** 에 `simultaneousGesture` 로 붙인다. 조상의
제스처는 자식의 탭과 경쟁하지 않고 공존한다. 스크롤 영역은 오프셋을 받아 보여주기만 한다.

**예방 규칙.**
- 겹치는 레이어에 인터랙션을 넣을 때 "ZStack 순서가 히트 테스트 순서다"를 믿지 않는다 — 아래
  형제에 제스처/플랫폼 뷰가 있으면 실측한다. 여기서 결정적이었던 건 **위치만 다른 네 개의
  타깃(상·중·하·안)을 한 프로브에 놓은 것**이었다. 한 개로는 "안 된다"만 알고, 넷이면
  "어디서 안 되는지"가 나와 원인이 갈린다.
- 구조를 바꿨으면 **단일 탭이 아니라 기능 게이트 전체**를 다시 돌린다. 여기서도 단일 탭 검증
  (사이드 메뉴)은 통과했고, 게이트가 25/92 를 보고해서야 드러났다.

## 2026-08-23 — "44pt 로 해결"은 드라이버의 4% 좌표 드리프트를 덮은 것이었다

**실패 모드.** 오늘 아침 18pt 타깃이 안 눌려 44pt 로 키우니 통과했고, "원본이 HIG 최소 터치
타깃을 안 지킨다"로 결론지었다. 오후에 스크롤을 넣자 하단 타깃이 다시 죽었고, `답글 달기`를
누르면 `게시물 만들기` 가 발화했다 — 탭이 **보고된 프레임보다 ~30pt 아래**에 착지하고 있었다.

**원인.** iPhone 12/13 mini 는 1125×2436 으로 렌더해 1080×2340 패널로 다운샘플한다. AXe 는
패널 기준으로 점→픽셀을 계산해 요청 y×(2436/2340) 에 탭이 떨어진다 — y 에 비례해 밀리고,
y=700 에서 ~29pt. 44pt 는 그 오차를 ±22 로 덮었을 뿐이다. **실기기 손가락에는 없는 문제**다.

**검출 신호.** (1) 성공/실패가 y 에 따라 갈린다 (2) 실패한 탭이 "아무 일도 안 함"이 아니라
**다른 타깃을 발화**한다 (3) 같은 코드가 화면 중간에선 되고 가장자리에선 안 된다. 셋 다 "프레임은
맞는데 터치가 엉뚱한 곳에 떨어진다"의 신호다.

**예방 규칙.**
- 드라이버의 탭은 **보고된 프레임의 중심을 보정 좌표로** 친다. 보정 계수는 하드코딩이 아니라
  런타임 산출(시뮬레이터 스크린샷 높이 ÷ 논리 높이 → 렌더 스케일 반올림 → 패널/렌더 비율).
- "크기를 키우니 된다"는 해결이 아니라 **오차를 감추는 것일 수 있다**. 키워서 통과하면, 왜
...
## 2026-08-24 검증 명령이 exit code를 삼키면 검증이 아니다

- **실패 모드**: 전체 스위트를 `(runner || true) | tail -15`로 백그라운드 실행해 exit code와 unittest 요약("Ran N tests / OK")을 모두 잃었고, tail에 걸린 테스트 픽스처 출력("FAKE ... drift")을 실제 오류로 오인할 뻔했다. 같은 시간대에 디스크 만성 부족(ENOSPC)으로 백그라운드 task output 파일이 비어 돌아와 혼선이 겹쳤다.
- **검출 신호**: 출력에 "Ran N tests" 요약이 없다. task 완료 알림은 왔는데 output 파일이 0바이트다. `df -h`에서 가용량이 수백 MB 이하.
- **방지 규칙**: 검증 실행은 로그를 파일로 직접 쓰고(`runner >log 2>&1; echo exit=$? >>log`) 요약과 exit 라인을 그 파일에서 읽는다. `|| true`·중간 pipe로 exit code를 가리지 않는다. 백그라운드 output이 비어 있으면 결과 유실이지 성공이 아니며, 먼저 디스크부터 확인한다.
