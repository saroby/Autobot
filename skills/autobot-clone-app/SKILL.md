...
`views.json` 의 화면마다 렌더 → 구조 diff → 구조 추출 감사 → 대조를 한 번에 돌리고, 요소 누락·렌더 실패·**구조 추출 누락**·**시각 mismatch 초과**가 하나라도 있으면 **non-zero** 로 끝난다. `<NN>` 을 손으로 갈아 끼우며 화면마다 세 명령을 반복하지 않는다 — 그렇게 하면 어느 화면을 빠뜨렸는지 아무도 모른다. 아래 1–3은 `verify` 가 화면마다 실행하는 내용이며, **한 화면만 진단할 때** 직접 쓴다.
...
4b. **구조가 실제로 뽑혔는지도 센다 — 요소 누락·픽셀 유사도와는 다른 축이다.** 위 두 축은 둘 다 충실도다: 측정한 요소가 다 있는가, 원본과 닮았는가. 어느 쪽도 생성된 SwiftUI 에 구조가 있는지는 묻지 않는다 — 카드 30장을 독립된 30개 절대좌표 블록으로 찍어낸 결과물도 둘 다 만점으로 통과하고, 그것은 사람이 편집할 수 있는 코드가 아니다. `clone_structure.py`(Step 5, `codegen`)가 반복 그룹을 감지해 `.autobot/clone/structure/<NN>.json` 에 초안을 쓰고, `clone_view_codegen.py` 의 `repeat_units()` 가 확정된 그룹을 컴포넌트 + `ForEach` 로 뽑는다 — 하지만 `repeat_units()` 에는 그룹을 조용히 건너뛰는 경로가 있다(id 를 다른 그룹이 먼저 채간 경우, crop 이 id 하나를 지워 부분집합이 깨진 경우, 스크롤 경계를 넘는 경우). 건너뛴 그룹은 화면을 평평한 replay 로 조용히 되돌리고, 위 두 축은 그 퇴행을 볼 방법이 없다.
   ```bash
   python3 scripts/clone_structure_audit.py .autobot/clone <NN> <View>
   ```
   규칙은 비율이 아니라 원칙이다: **선언된 그룹은 전부 추출된 컴포넌트로 나타나야 한다** — 하나라도 빠지면 `exit 1` 이고 어느 컴포넌트가 빠졌는지 이름으로 나온다. 아무도 측정한 적 없는 퍼센트 문턱을 새로 만드는 대신 크리스프한 규칙을 쓴다. 감지된 그룹이 없는 화면은 애초에 뽑을 게 없으므로 실패가 아니다. 사람이 화면을 손으로 가져간 경우(생성 마커를 지운 `.swift`)는 파이프라인의 다른 소유권 경계와 똑같이 이 게이트에서도 면제다 — 이제 그 사람 것이고, 추출을 요구할 수 없다.
   **이 게이트가 주장하지 않는 것**: 추출된 컴포넌트의 내용이 옳다는 것도, 컴포넌트 이름이 좋다는 것도, 반복 감지 자체가 맞았다는 것도 아니다. 오직 "선언되었다면 뽑혔는가"만 본다.
