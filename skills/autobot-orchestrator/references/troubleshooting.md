...
| "build-state.json 없음" | /autobot:resume 실행했지만 이전 빌드 없음 | `/autobot:mvp`로 새 빌드 시작 |
...
| architecture.md가 불완전 | 아이디어가 너무 모호 | 더 구체적인 아이디어로 `/autobot:mvp` 재시작 |

### Phase 3: 프로젝트 생성 실패

| 증상 | 원인 | 해결 |
|------|------|------|
| ".xcodeproj not found" | xcodegen 없고 pbxproj 생성 실패 | `python3 --version` 확인 (3.8+ 필요) 후 `/autobot:resume 3` |
| "xcodegen generate failed" | project.yml 문법 오류 | `xcodegen` 없이 fallback으로 재시도: `rm project.yml && /autobot:resume 3` |
| 번들 ID 충돌 | 이미 다른 프로젝트에서 사용 중 | `build-state.json`의 bundleId 수정 후 `/autobot:resume 3` |

### Phase 4: 코드 생성 실패

| 증상 | 원인 | 해결 |
|------|------|------|
| `<AppName>/Models/` 파일이 변경됨 | 에이전트가 금지 규칙 위반 | Gate 4→5에서 자동 복원 (`snapshot-contracts.sh restore`) |
| `<AppName>/Views/` 비어있음 | ui-builder 에이전트 실패 또는 프로젝트 루트에 잘못 생성 | `Views/`가 `<AppName>/Views/`에 있는지 확인 후 `/autobot:resume 4` |
| `<AppName>/Services/` 비어있음 | data-engineer 에이전트 실패 또는 경로 오류 | `/autobot:resume 4` |
...
# 그 후 /autobot:resume 또는 /autobot:mvp 재실행
```
