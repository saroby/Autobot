...
두 파일이 모두 없으면 깨끗한 첫 빌드이므로 그대로 진행한다 (이 경우 step 4 는 건너뛴다).
...
phase-specific 또는 active 파일 중 **최소 하나라도 적용했다면**, build-log.jsonl 에 `learning_applied` 이벤트를 기록한다:
...
두 경우 모두 이 문서의 절차를 그대로 따르되, **phase 번호·파일 경로·agent 이름** 세 값만 바뀐다.
