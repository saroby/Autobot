...
**라벨에서 온 후보는 역할에서 온 후보보다 약하고, 그 차이를 LLM 이 메운다.** 라벨이 없는 컨트롤은 후보로 나오지도, 검사되지도 않는다. 그래서 `WARN: role-blind screen` 을 본 화면에서는(그리고 라벨 기반 후보를 누를 때는 언제나) 탭 전에 매번:
...
4. **탐색 목적에 맞는 것만 고른다.** 탭바·상단 네비·리스트 항목·상세 진입이 목표다. 광고·프로모션 배너는 앱 밖으로 나가므로 건너뛴다.
