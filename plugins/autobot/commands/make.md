...
**Gate 0→1 검증 + Phase 완료 기록 (단일 호출):**
```bash
bash "$CLAUDE_PLUGIN_ROOT/scripts/pipeline.sh" advance-phase --phase 0
```

`advance-phase`는 outgoing gate(여기서는 `0->1`)를 먼저 실행하고, 통과한 경우에만 phase를 `completed`로 마킹한다. 실패 시 phase는 `failed`로 기록되어 resume 흐름이 안전하게 받아낸다.
...
3. 검증 결과 `backend_required`가 `false`인데 위 조건에 해당하면:
   - `build-state.json`의 `backend_required`를 `true`로 **오버라이드** (직접 JSON 편집 금지, runtime을 통해 기록):
     ```bash
     bash "$CLAUDE_PLUGIN_ROOT/scripts/pipeline.sh" set-flag \
       --key backend_required --value true \
       --reason "architect-review override: external AI API detected"
     ```
...
```bash
bash "$CLAUDE_PLUGIN_ROOT/scripts/pipeline.sh" advance-phase --phase 3
```
...
각 에이전트는 학습 파일을 적용한 직후 `build-log.sh --event learning_applied`를 호출해야 한다 (인스트럭션은 `agents/*.md`에 명시). 이 호출이 `phases.4.learningsConsumed`에 누적되며, Gate 4→5의 `phase4_agents_consumed_learnings` 체크가 ui-builder/data-engineer 양쪽 기록을 강제로 검사한다 — 호출을 빠뜨리면 gate 거부.

**에이전트 완료 후 파일 소유권 검증 (위반은 자동으로 `phases.4.sandbox.violations`에 기록됨):**
```bash
bash "$CLAUDE_PLUGIN_ROOT/scripts/agent-sandbox.sh" after --agent ui-builder --app-name "<AppName>" --phase 4
bash "$CLAUDE_PLUGIN_ROOT/scripts/agent-sandbox.sh" after --agent data-engineer --app-name "<AppName>" --phase 4
# (backend_required == true일 때)
bash "$CLAUDE_PLUGIN_ROOT/scripts/agent-sandbox.sh" after --agent backend-engineer --app-name "<AppName>" --phase 4
```

`agent-sandbox.sh after`는 위반을 자동으로 `phases.4.sandbox.violations`에 state로 기록한다 (Gate 4→5의 `sandbox_clean` 체크가 이를 검사). 위반이 발견되면 위반 파일을 삭제하고 해당 에이전트만 재실행한 뒤 다시 `after`를 호출한다.

**→ Gate 4→5**: 파일 존재 + `<AppName>/Models/` 체크섬 무결성 + sandbox 위반 0건. 체크섬 불일치 시 `.autobot/contracts/phase-1-models/` snapshot으로 복원한다.
...
quality-engineer는 빌드 성공이 확정되는 즉시 `phases.5.metadata.build_succeeded=true`를 기록해야 한다 (`autobot-integration-build` 스킬 참조). 그 다음 한 번에 검증 + 완료 마킹:
...
Circuit breaker가 트립한 경우 runtime이 Phase 7을 자동으로 `in_progress`로 마킹하고, 미완료 phase들을 `skipped`(`skipReason: "circuit breaker tripped on phase N"`)로 일괄 마킹한다. 이 경우엔 `start-phase --phase 7`을 별도로 호출할 필요 없이 바로 회고 산출물 생성으로 진입한다.
...
**빌드 잠금 해제 + 회고 종료 (Phase 7은 outgoing gate가 없어 advance-phase가 단순 완료 마킹으로 동작):**
```bash
rm -f ".autobot/build.lock"
bash "$CLAUDE_PLUGIN_ROOT/scripts/pipeline.sh" advance-phase --phase 7 --detail "Build pipeline finished"
```
