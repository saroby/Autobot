...
| 증상 | 원인 | 해결 |
|------|------|------|
| `<AppName>/Models/` 파일이 변경됨 | 에이전트가 금지 규칙 위반 | Gate 4→5에서 자동 복원 (`git checkout -- <AppName>/Models/`) |
...
| 5회 재시도 후에도 실패 | 구조적 불일치 | Phase 4부터 재시도: `/autobot:resume 3` |
...
| Phase 5가 무한 루프 | 에이전트가 같은 에러를 반복 수정 | 5회 재시도 후 자동 중단됨, `/autobot:resume 3`으로 코드 재생성 |
