...
3. **Read Design Spec (if exists)**: Check if `.autobot/design-spec.md` exists. If so, read it for:
   ...
   - Check `.autobot/designs/*.png` for visual mockup images of each screen
   This file is generated by Phase 1.5 (Stitch MCP) and provides concrete visual direction. If it doesn't exist, proceed with architecture.md alone.
...
7. **Create Each Screen**: One Swift file per screen in `<AppName>/Views/Screens/`
   - If design-spec.md exists, reference the corresponding screen's design tokens and layout notes
   - If `.autobot/designs/<ScreenName>.png` exists, use it as visual reference for layout decisions
