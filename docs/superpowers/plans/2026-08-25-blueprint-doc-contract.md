...
| 대상 | 사람이 소유권을 주장하는 방법 | 어디서 성립하나 |
|---|---|---|
| 항목 | 근거 라벨을 `우리 결정` 으로 바꾼다 | 모든 항목 |
| 생성 뷰 (clone 스킬) | `// Generated by` 줄을 지운다 | 모든 생성 뷰 |
| **노트** | **`⟦auto:…⟧` 마커를 지운다** | **`우리 결정` 항목에서만** |

세 번째는 **코드 변경이 필요 없다.** 마커 없는 `>` 줄은 파서가 이미 본문으로 남긴다. 새 규칙을 만드는 게 아니라 이미 있는 동작에 이름을 붙이는 것이다.

**단, 세 번째는 앞의 둘과 달리 일반 규칙이 아니다** (최종 리뷰 I-6). 마커를 지운 줄은 **본문이 되고**, `관찰`·`공개자료`·`가설(미검증)` 항목의 본문은 새 관찰이 있는 회차마다 `merge_items` 가 통째로 교체한다. 그러니 마커 삭제가 글을 지키는 것은 본문 교체가 없는 `우리 결정` 항목에서뿐이다.

`conflict` 노트는 `우리 결정` 항목에만 붙으므로 `NOTE_KEEP_HINT` 가 가르치는 문구 자체는 참이다. 그러나 `absent` 노트는 `관찰` 항목에 붙고, 사람이 거기 "확인함, 다음 회차에 삭제" 같은 주석을 다는 것이 가장 자연스럽다 — 그 자리에서 배운 규칙을 그대로 적용하면 글이 사라진다. `absent` 노트에는 같은 안내를 붙이지 않는 이유가 이것이다. `관찰` 항목에 사람의 글을 남기려면 **라벨을 `우리 결정` 으로 바꾸는 것이 유일한 방법이다.**
...
## 개정 — 최종 리뷰 수정 웨이브 (2026-08-25)

`.superpowers/sdd/2026-08-25-blueprint-doc-contract/final-review.md` 의 Critical 3건·Important 4건과 원장 필수 5건을 닫으면서 계약이 바뀌었다. 위 태스크 본문의 인터페이스 서술은 이 절 이전의 상태를 기록한 이력이고, **아래가 최종 계약이다.**

리뷰의 진단: 병합 층은 견고했고 구멍은 전부 파서·렌더러 경계에 있었다. 소유권 판정이 항목 단위로 내려지려면 사람이 쓴 것이 먼저 항목 안에 온전히 들어와 있어야 하는데, 파서가 세 종류의 사람 입력을 항목 밖으로 흘렸다 — **라벨이 소유권을 정하기도 전에 글이 사라졌다.**
...
- `parse_document(text) -> Document` · `render_document(document) -> str` — 이 둘은 역함수다.
- `read_doc(path) -> Document` (없는 파일은 `Document()`) · `write_doc(path, document) -> None`.
- `render_items(items)` 에서 `heading` 인자를 없앴다. 문서 제목 `# 기능` 은 머리말의 첫 줄일 뿐이다.
- `parse_items(text) -> list[Item]` 는 `parse_document(text).items` 로 남는다.

첫 항목 앞의 줄을 버리면 `read_doc → merge_items → write_doc` 이 역함수가 아니게 되고, 저장 한 번에 제목과 사람이 쓴 안내문이 사라진다. 리뷰가 제시한 두 대안 중 "`check` 가 `WARN:` 으로 알린다" 는 유실을 **알릴** 뿐 막지 못하므로 택하지 않았다.

### 2. 이미지 줄은 줄 전체일 때만 이미지다 (C-1)

```python
_IMAGE_LINE = re.compile(r"^\s*(?:<img\s[^>]*>\s*)+$")
```

- `Item.images` 는 `src` 목록이 아니라 **줄 원문** 목록이다. 사람이 정한 `alt`·`width` 는 되살릴 방법이 없으므로 애초에 버리지 않는다.
- 렌더는 저장된 줄을 그대로 다시 쓴다. 폭 정책은 생산자 헬퍼로 옮겼다: `image_line(src)` → `<img src="…" width="220">`.

`.search` 로 줄 아무 데나 찾던 때는 `설명 <img …> 설명` 한 줄이 통째로 이미지로 흡수돼 산문이 사라졌다. 스펙 규칙 6 이 이미지를 문서 어디에나 흔하게 만들므로, 이미지 옆에 설명을 붙이는 것은 예외가 아니라 기본 사용법이다.

### 3. 항목이 되지 못한 `##` 줄은 거부한다 (C-3, M-16)
...
- 접두사는 스펙이 정한 다섯 개뿐이다. Task 1 이 Task 9 로 넘겼다가 이행되지 않은 약속을 여기서 닫는다.
- `malformed_headings(text) -> list[tuple[int, str]]` 를 `check` 가 **ERROR 로 거부한다.** 이런 줄은 조용히 직전 항목 본문이 되고, 그 항목이 `관찰` 이면 다음 병합이 지운다. 흡수는 무성(無聲)이면 안 된다.
- 코드펜스 안의 `## F-999 …` 는 항목이 아니다. 파싱도 `malformed_headings` 도 펜스를 존중한다.
- `write_doc` 은 원본 파일에 `## ` 줄이 있는데 항목이 0개면 `DocumentWriteRefused` 를 올린다. 항목 0개는 "빈 문서" 일 수도 "못 읽었다" 일 수도 있고, 후자에 쓰기를 허용하면 문서가 빈 파일이 된다.
- `parse_document` 의 docstring 에서 "그 밖의 줄은 무시한다" 를 사실(흡수한다)로 고쳤다.

### 4. 본문 없는 회차는 관찰 항목도 비우지 못한다 (I-4)
...
`우리 결정` 경로에만 있던 가드를 `관찰` 경로에도 둔다. 한 화면에서 텍스트 추출이 실패한 회차가 누적된 본문·이미지·참조를 비우는 것은 스펙 규칙 3("부분 관찰은 부분이라고 말한다")과 어긋난다. 지킬 것이 없는 항목은 새 관찰을 그대로 받는다.

### 5. 중복 ID 는 문서가 가질 수 없다 (I-5)

`duplicate_ids(items) -> list[str]` 를 `check` 가 ERROR 로 거부한다. 병합 의미론은 정하지 않는다 — 문서가 중복을 갖지 못하게 막으면 된다. ID 는 병합의 키이고 다른 문서가 참조하는 주소이므로, 중복이 있으면 뒤의 항목이 이번 회차에 관찰되고도 `없음` 표시를 받는다.

### 6. `check` 가 검사하는 것 (Task 10 갱신)

`blueprint_merge.py check <doc.md>` 는 세 가지를 모두 보고한 뒤 하나라도 있으면 `1` 을 돌려준다.

1. 항목이 되지 못한 `## ` 줄
2. 중복 항목 ID
3. 근거 라벨이 없거나 알 수 없는 항목

### 7. 이음매 테스트 (I-7)

`tests/test_blueprint_merge.py` 의 `TestTheWholePath` 가 `parse_document → merge_items → render_document → parse_document` 전 구간을 지난다. 두 모듈이 실제로 만나는 유일한 경로이고, 그 경로가 무테스트였던 것이 세 Critical 이 리뷰 10회를 살아남은 이유다.
