...
- Autobot 의 포지셔닝을 "iOS 앱 빌더"에서 "기획·빌드·배포·스토어 운영을 아우르는 종합 개발 플러그인"으로 재정의했다. 코드 변경 없음 — README, plugin.json description, SOUL.md 만 갱신. 빌드 파이프라인의 현재 구현 타깃은 여전히 iOS 26+ 이고, 독립 명령(ssot·screen·make·feedback 등)은 원래부터 플랫폼 무관이다.
...
- 정지 화면 검사기는 정지 화면만 검증한다는 것이 이 회차의 실측이다 — 구조 diff 와 픽셀 지표가 전부 통과한 화면에서 탭 4건 중 3건이 아무 데도 가지 않았다.

### Added — 재현의 첫 판을 측정이 쓴다
- `scripts/clone_view_codegen.py` — 측정 JSON 하나당 SwiftUI 뷰 하나. 측정된 프레임·색·글자 크기·접근성 라벨을 그대로 재생한다. `observe` 가 끝에 자동 실행하며, 파일 첫 줄의 생성 마커를 지우면 그 화면은 다시 생성되지 않는다(손질본 소유권 경계).
- 그 전에는 화면마다 빈 자리표시자에서 시작해 `verify` 가 31개 화면 전부에 "요소 전부 누락"이라는 같은 말만 했다. 실측: 구조 diff 가 **27/28 화면에서 요소 누락 0** 으로 통과한다(직전 회차에는 유일한 손작성 화면조차 163개 중 142개 누락).
- 생성 뷰는 **그림과 조작을 분리한다.** 측정 재생 레이어는 아무 터치도 받지 않고, 관측된 전이만 담은 투명한 액션 레이어가 그 위에 그려진다. 겹쳐 쌓인 166개 요소를 각각 탭 가능하게 만들면 접근성 트리는 좌표를 정확히 보고하는데 어떤 터치도 도달하지 않는 컨트롤이 생긴다.
- 탭 타깃은 측정 중심을 유지한 채 **44pt(HIG 최소)** 로 키운다. 원본이 3pt 짜리 시트 손잡이나 18pt 오버플로 점을 쓰기 때문에, 프레임을 그대로 재생하면 재현본은 조작 자체가 불가능하다(28pt 로는 부족했다).
- 라벨은 컨테이너 체인을 따라 반복되므로(Threads 피드의 `게시 옵션` 은 한 프레임에 5개) 액션 요소마다 `clone-action:<label>` 식별자를 준다. 식별자가 없으면 드라이버가 "5개가 매치되고 AXUniqueId 가 없다"로 탭을 거부한다.
- 관측된 전이의 라벨이 그 캡처에 없으면(라이브 피드는 캡처마다 다른 글이다) **기록된 탭 좌표에** 타깃을 만든다. 측정이 아예 없는 화면도 전이는 유지되므로 flow 가 그 화면을 통과한다.
- 합성한 타깃은 캡처가 담고 있던 요소보다 약한 증거다. 생성기가 **몇 개를 어디에 만들었는지 이름으로 내고**, 접근성 식별자도 `clone-action-synth:` 로 구분해 기능 워크가 `42/42 wired (6 of them on a target synthesised ...)` 로 귀속한다. 세지 않으면 "전이 전부 배선"이 실제보다 강하게 읽힌다.
- `functional` 이 증명하는 것은 **배선**이다 — 라우터 표·뷰의 탭 타깃·검사하는 엣지 목록이 모두 `observed_transitions` 한 곳에서 나오므로, 대상 앱과 동작이 같은지를 재는 것이 아니다. 그 대신 기계적으로 깨지는 것(중복 라벨·닿지 않는 타깃·가로채는 레이어)을 정확히 잡는다.
- `polish` 의 state→측정 조인이 생성기의 조인을 그대로 쓴다. 캡처 선택이 "가장 풍부한 것"으로 바뀐 뒤로는 두 번째 구현이 화면을 **자기 자신의 다른 캡처**와 대조하고 있었다.

### Added — 크롬 전이 추론 (Codex 교차 검토 반영)
- **도착 화면에도 그대로 남아 있는(persistent) 컨트롤**은 한 화면에서 관측한 전이를 같은 컨트롤(라벨+역할+8pt 프레임 격자)이 있는 다른 화면에도 적용한다. 탭바가 전형이다 — 화면마다 따로 관측해야 살아나던 버튼이 한 번의 관측으로 전부 살아난다. 뒤로가기·닫기는 도착 화면에 없으므로 **구조적으로** 제외된다(라벨 사전 없음). 실측: 관측 105건에 추론 47건, 기능 게이트 73/92 → **125/129**.
- 추론은 앱에서 본 적 없는 전이다. 라우터에 `// inferred` 로 표시되고, 뷰·워커 모두 `clone_flow_codegen.all_transitions` 한 곳에서 읽으며, 게이트가 "N건 추론, M건 배선"을 따로 센다 — 통과는 "추론과 일치"이지 "앱과 일치"가 아니다.
- 겹치는 합성 타깃은 둘 다 제거한다 — 어느 것이 눌릴지 알 수 없고 둘 다 캡처와 맞지 않는다.

### Fixed — 기능 게이트의 드라이버가 4% 어긋난 좌표로 탭하고 있었다
- iPhone 12/13 mini 시뮬레이터는 1125×2436 렌더를 1080×2340 패널로 다운샘플하는데 AXe 는 패널 기준으로 점→픽셀을 계산한다. 요청 y 가 클수록 탭이 아래로 밀려(y=700 → 실제 729) 하단 타깃을 빗나가 탭바를 맞췄다. 이전 회차의 "44pt 타깃으로 해결"은 이 오차를 **덮은** 것이었다. 워커가 요소 프레임을 읽어 패널/렌더 비율(스크린샷 크기 ÷ 논리 크기, 런타임 산출)로 보정한 좌표로 탭한다. 실기기 손가락에는 없는 문제라 재현본은 불변.
- 중단된 `observe` 가 새 화면을 flow 에 남겨도 라우터·뷰는 재생성되지 않아 워커만 아는 엣지가 BROKEN 으로 보였다. `observe` 가 manifest(기존 이름 유지 병합)·라우터·뷰를 **항상** 함께 재생성하고, 워커는 views.json 의 상태 집합만 기대한다.

### Fixed — 재현본이 "되다 만" 앱이었다 (사용자 리뷰: 스크롤 없음, 버튼 죽음)
- **탐험이 깊은 화면에서 길을 잃고 끝났다.** `next-tap` 은 관측된 전이로만 경로를 찾으므로 설정 깊은 곳에서 남은 37개 화면으로 돌아갈 길이 없었고, 거기서 종료했다 — 타깃 522개 중 100개만 탐험된 직접 원인이자 버튼 대부분이 죽어 있던 직접 원인(관측이 없으면 배선도 없다). 이제 앱을 재시작해 초기 화면에서 이어간다(`CLONE_EXPLORE_MAX_RESTART`, 기본 8).
- **텍스트 입력을 안 했다.** 검색 화면까지는 가는데 결과 화면은 키보드 너머에만 있어 영원히 미관측이었다. 탭할 것이 없을 때 화면의 텍스트 필드에 probe 를 한 번 입력하고(state×field 당 한 번) 캡처한다. 입력값은 로그에 남기지 않는다 — `type` 의 기존 규칙. flow 에 `type` 이벤트(`field`, `from/to_statekey`)가 생기며 모든 reader 가 무해하게 통과한다.
- **no-op 탭 한 번이 ~20초였다.** settle 루프의 종료 조건이 "화면이 바뀌었다" 하나뿐이라 아무 일도 안 일어난 탭은 10회 폴링(WDA `/source` 10번)을 끝까지 돌았다. 실측 p90 26.8s vs 중앙값 4.5s. 연속 3회 동일이면 no-op 으로 확정하고, 폴링당 python 2회를 `identity` 1회로 합쳤다.
- **스크롤이 없었다.** 122개 캡처 중 109개에 스크롤 컨테이너가 있는데 재현본은 고정 스택이었다. 접근성 트리가 크롬과 콘텐츠를 이미 분리해 주므로(Threads 상단바·탭바는 피드의 자손이 아니다) 그대로 나눠 렌더한다 — 콘텐츠는 스크롤 영역 안에 컨테이너 기준 좌표로, 크롬은 밖에 고정, 탭 타깃도 각자의 레이어에.
- **플랫폼 `ScrollView` 는 쓰지 않는다.** iOS 26 시뮬레이터에서 ZStack 안의 `ScrollView` 아래 깔린 크롬은 화면 상·하단 가장자리에서 탭을 받지 못했다 — 탭바와 상단바, 클론에 가장 중요한 컨트롤이다. 4-타깃 프로브로 실측했고 `.zIndex`·`.overlay`·`Button`·`highPriorityGesture`·`.ignoresSafeArea`·`.scrollEdgeEffectHidden` 전부 소용없었다. 드래그 오프셋 방식(`CloneScrollArea`, 클램프만·관성 없음)으로 교체했다.
- **그 드래그 제스처를 전체 화면 형제에 붙여도 같은 문제가 났다.** 제스처를 떼면 탭바가 다시 살아나는 것을 실측. 제스처는 **화면 루트(모든 타깃의 조상)** 에 `simultaneousGesture` 로 붙인다(`CloneScrollDrag`) — 조상의 제스처는 자식 탭과 경쟁하지 않고 공존한다. 탭바·상단바·피드 내 타깃·스와이프 네 가지를 한 빌드에서 실측했다.
- **다른 기기 크기에서 어긋났다.** 좌표가 관측 기기(375×812) 절대값이라 루트 캔버스를 기기 크기에 aspect-fit 한다. 같은 크기면 identity 라 기존 렌더·검증은 바이트 단위로 그대로.

### Fixed — 뒤로가기가 파이프라인을 막았다
- 화면이 47개로 늘자 `generate` 가 `ambiguous transition from '…' for action '돌아가기': 3 destinations` 로 거부했다. 관측은 정확했다 — **뒤로가기는 어디서 왔느냐에 따라 목적지가 다르다**. `state × action → state` 표가 그것을 표현할 수 없었을 뿐이다.
- 한 액션의 목적지가 여럿인데 **그 전부가 이 화면에 도달했던 출발지**라면 모순이 아니라 pop 이다. 앱이 스택을 갖고 있으므로 라우터도 갖는다(`history.append` / `history.popLast`). 와 본 적 없는 곳이 섞여 있으면 **여전히 모순으로 거부**한다 — 추측으로 넘어가지 않는 경계를 지켰다.

### Fixed — 자원 게이트가 출발선만 쟀다
- `doctor` 가 `disk free 3538MB (minimum 2048MB)` 로 통과시킨 실행이 WDA/Xcode 빌드·캡처·렌더 캐시로 그 3.5GB 를 전부 쓰고 ENOSPC 로 죽었다. 게이트는 거짓말하지 않았다 — 시작 시점에는 충분했다. 임계값을 실측 소비량 기준으로 다시 잡았다(2048 → 6144MB)。

### Changed — 터널 인증을 스킬 시작 시점에 묻는다
- `clone_run.sh observe` 가 기기를 확정한 직후 RemoteXPC 터널을 먼저 세운다. 터미널이면 `sudo -v` 로, 아니면 **macOS 관리자 대화상자**로 그 자리에서 묻는다. 그 대화상자는 원래도 떴지만 몇 분 뒤 `doctor` 안에서였고, 사용자가 보고 있지 않을 때 조용히 떴다가 시간 초과로 죽었다.
- `device_wda.sh tunnel-status <udid>` / `tunnel-start <udid>` 신설. `tunnel-status` 는 프로필이 **다른 기기**를 가리키면 통과가 아니라 exit 2 로 거부한다 — CoreDevice 식별자를 하드웨어 UDID 자리에 넘겼을 때 iOS 26 기기를 "터널 불필요"로 조용히 통과시켰다.

### Added — 픽셀 폴리싱 2차: 아이콘, 정직한 지표, 빠른 반복
- **`device_compare.py --mask-assets <manifest>`** — 이 화면이 그리는 캡처 crop 을 전부 제외하고 잰다. crop 은 대조 대상인 바로 그 원본에서 잘렸으므로 그 영역은 구조적으로 mismatch 0 이고, 포함하면 "재현" 대신 "crop 배치"를 재게 된다. `polish` 가 항상 함께 내며, 로그가 제외 면적을 먼저 말한다(예: "32 region(s), 48.1% of the screen").
- **아이콘이 재현본에 통째로 없었다.** 대조 이미지를 눈으로 보고 발견했다 — 하트·댓글·리포스트·공유·상단바·탭바가 전부 비어 있었다. 아이콘은 접근성 트리에 **그림이 아니라 설명**(`좋아요. 226명이…`)으로만 존재해 어떤 측정으로도 재현할 수 없다. `device_assets` 의 crop 추출을 아이콘 크기의 **잎 컨트롤**까지 넓혔다(`AXLink` 는 글자라 제외, 측정된 static text 와 라벨이 같은 컨트롤도 제외 — 같은 글자를 두 번 그리게 된다). 고유 crop 76 → 387개, 5.3MB.
- **`clone_run.sh polish <화면>`** — 측정 stem 이나 뷰 이름 하나만 돌린다. 매번 28개 화면을 다시 재는 루프는 두 번 돌리지 않게 된다. 대조 단계는 시뮬레이터가 필요 없어 병렬로 돈다(`CLONE_COMPARE_WORKERS`, 기본 4).

### Fixed — 화면 밖으로 뻗은 요소의 crop 을 프레임 전체로 늘렸다
- Threads 의 `AdditionalDimmingOverlay` 는 `x=-120` 에서 시작하는 615x286 요소다. crop 은 캡처 안으로 잘리는데 그것을 원래 프레임 전체에 `.resizable()` 로 펴 바르면 안의 픽셀이 확대된다 — 피드의 14pt 한 줄이 배너만 한 글자로 나왔다. 이제 **crop 은 잘라낸 영역에** 그린다. 그림은 자기 프레임을 가진 별도 레코드가 되고, 요소는 구조 diff 를 위해 자기 라벨·프레임을 그대로 유지한다.

### Fixed — 라벨 없는 래퍼 안의 로고·글리프가 빈 공간으로 렌더됐다
- Threads 워드마크는 **빈 `AXOther` 잎** 안에 산다. 라벨이 없어 식별할 수 없고, 컨트롤 역할도 아니며, 그 영역을 덮고 있어 uncovered 스캔까지 조용하게 만든다. 측정이 가진 것은 단색 fill 뿐이라 상단바가 빈 공간이었다.
- 기준을 하나 추가했다 — **평평하지 않은 픽셀은 fill 로 재현할 수 없다.** 라벨 없는 아이콘 크기 잎은 그 영역이 균일하지 않을 때만 crop 한다(구분선·카드 배경 같은 평평한 것은 기존대로 측정된 fill).

### Fixed — 장식 이미지가 재현본에서 통째로 빠졌다
- Threads 게시물 아바타는 **접근성 트리에 없다**(장식 요소). 그것을 잡으라고 만든 `uncoveredRegions` 스캔이 0건을 보고했는데, 커버리지 계산에 **컨테이너까지 세고 있었기** 때문이다 — 행 컨테이너가 아바타 자리를 덮으니 "설명됨"으로 처리됐다. 컨테이너는 자식의 영역을 덮지만 거기에 아무것도 그리지 않는다. 이제 **잎 요소만** 픽셀을 설명한다.
- `device_assets` 가 그렇게 드러난 영역도 crop 하고(manifest 에 `role: uncoveredRegion`, 음수 index 로 기록), 생성 뷰가 그 자리에 그린다. 재현할 수 없는 픽셀을 재현한 척하지 않는 방법은 세어서 말하는 것이고, `--mask-assets` 가 지표에서 그 영역을 뺀다.

### Fixed — 앱이 뜨기 전에 찍은 화면을 "요소 전부 누락"으로 기록했다
- 픽셀이 조용한 것과 앱이 뜬 것은 다르다 — 홈 화면도 조용하다. 한 회차에서 화면 5개가 접근성 트리가 통째로 빈 채 기록됐고 구조 diff 는 각각을 화면 전체 누락으로 읽었다. 이제 트리가 이 앱 이름을 말할 때까지 기다린 뒤 한 번 다시 찍는다(이미 뽑던 트리라 추가 비용 없음).

### Fixed — 재현본 빌드가 4분이었다
- 생성 뷰의 거대한 Swift 배열 리터럴을 **JSON 문자열 + 런타임 디코드**로 교체했다. 기본값 인자가 있는 초기화자의 배열 리터럴은 Swift 타입 체커에서 거의 제곱 시간이고, 170개짜리 화면 하나가 13초였다. 전체 빌드 **4분 → 1분 49초**. 남은 시간은 CPU 가 아니라 디스크다(사용률 10%).

### Added — 픽셀 폴리싱 1차 (기능 게이트 통과 후)
- **추출해 둔 캡처 crop 을 실제로 그린다.** `--extract-assets` 가 이미 잘라 provenance 까지 남겨 뒀는데 아무도 쓰지 않아 모든 측정 이미지가 평균색 사각형이었다. `device_render.sh` 가 crop 을 앱 번들에 넣고(캐시 키에 파일명 = 내용 해시 포함) 생성 뷰가 이름으로 읽는다.
- **그 이득은 재현이 아니라 붙여넣기다 — 그렇게 보고한다.** crop 은 `raw/<stem>.png` 에서 잘렸고 대조도 같은 파일과 하므로, crop 영역의 mismatch 는 구조적으로 0 에 가깝다. 화면 픽셀의 **18~39%** 를 덮는 화면이 여럿이다. 생성기가 몇 개를 그렸는지 세어 말하고, 아래 수치도 쪼개서 낸다.
- 실측(공통 28개 화면, 세 갈래로 분리): 재생만 20.51% → 컨트롤 라벨 추가 20.60% → crop 추가 19.15%. **라벨은 −0.08pt(이득 없음), crop 이 +1.44pt 전부**다. 구조 diff 는 28/28 요소 누락 0 유지.
- crop 은 `.resizable()` 로만 그린다. `scaledToFill` 은 박스를 넘치고 **그 넘침이 접근성 프레임을 키워**(228pt → 286pt) 세 화면에서 요소 누락으로 읽혔다.
- 렌더 캐시에 상한(`CLONE_RENDER_CACHE_KEEP`, 기본 12). 이제 항목마다 crop 을 함께 담아 ~4MB 이고 소스를 고칠 때마다 새 항목이 생긴다 — 디스크가 차면 이 레포는 그것을 SwiftUI 컴파일 오류로 읽은 전례가 있다.

### Removed — 컨트롤에 제 라벨을 글자로 쓰던 규칙 (측정이 기각)
- 키보드 키 142개·버튼 79개가 high-mismatch 영역의 최대 두 덩어리라 라벨을 글자로 그려 봤다. 결과는 **평균 −0.08pt, 키보드 화면 −0.12pt / 나머지 +0.13pt — 순효과 0**.
- 대신 실제 결함을 만들었다: Threads 는 37x34pt 하트에 `좋아요. 226명이 이 게시물을 좋아합니다.` 를 라벨로 단다. 아이콘마다 잘린 한국어 두 줄이 그려졌다(23건). 막으려면 상자 용량·가정 글자 크기·이미지 가드 세 개가 더 필요했다.
- 그 규칙이 더해 주는 것처럼 보였던 문구(빈 상태 안내문 등)는 **전부 같은 화면의 측정된 `AXStaticText` 로 이미 그려진다**. 확인하고 되돌렸다.

### Fixed — 렌더가 화면 수만큼 같은 파일을 다시 컴파일했다
- `device_render.sh` 의 렌더 캐시 키에서 루트 뷰를 뺐다. swiftc 는 어차피 `Sources/` 전체를 컴파일하는데 키에 루트가 있어 31개 화면이 31회 풀 컴파일이었고, 뷰 하나를 고치면 31개 캐시가 전부 무효였다. 루트는 실행 시점에 `CLONE_ROOT_VIEW` 로 고른다(진입점이 dispatcher). 실측: 두 번째 화면 21.7s → 8.2s.
- 설치도 빌드가 바뀔 때만 한다. 대신 제자리 재실행은 "연속 2프레임 동일"이 **이전 화면**으로 만족될 수 있으므로, 실행 직전 프레임을 baseline 으로 잡아 그것과 같은 정지 프레임은 채택하지 않는다(끝내 그것뿐이면 실패가 아니라 WARN).
- 생성 뷰의 루트가 safe area 를 벗어난다. 그 전에는 측정 y 가 전부 8pt 아래로 렌더돼 구조 diff 허용치(8pt)와 정확히 같은 값에 걸려 있었다.
- `device_compare.py` 에 numpy 선택 경로. 화면당 22.0s → 7.7s 이고 **출력 PNG 는 바이트 동일**하다. 순수 파이썬 루프가 여전히 지표의 정의이며 numpy 가 없으면 그대로 쓴다(이 파일의 stdlib-only 계약 유지).

### Fixed — 세 번째 producer/reader 필드명 drift
- `clone_view_codegen.py` 가 전이를 `clone_flow_codegen.observed_transitions` 에서 그대로 가져온다. 처음에는 `type == "action"` 을 읽었는데 producer 가 쓰는 것은 `"tap"` 이었고, 픽스처도 같은 오해로 지어져 테스트 11개가 전부 통과한 채 실기기 로그에서는 탭이 하나도 배선되지 않았다. 라우터를 만드는 함수와 뷰의 탭을 만드는 함수가 같아야 어긋날 수 없다.

### Added — `/autobot:clone` 한방 실행 (사람이 화면 사이를 옮겨 주지 않는다)
- `scripts/clone_run.sh` — clone 의 기계적 두 반쪽. `observe` 는 workspace 준비 → 기기 게이트 → bundle ID 확인 → doctor → session → **앱 전체 탐험** → 측정·자산 crop → flow 맵 → `views.json` 을 질문 없이 수행하고, `verify` 는 뷰가 있는 화면마다 렌더 → 구조 diff → 대조를 돌려 요소 누락이 하나라도 있으면 non-zero 로 끝난다. 사이에 남는 것은 스크립트가 소유할 수 없는 저작(화면 스펙 `.md`, SwiftUI 뷰)뿐이다.
- `device_flow.py next-tap <flow.jsonl> <tree.xml>` — 지금 할 탭 **하나**를 답한다. 이 화면에 미탐험 후보가 있으면 그것, 없으면 미탐험 화면까지의 최단 관찰 경로의 첫 홉이다. 경로 홉은 이미 탭해 본 전이라 `todo` 가 제외하는 대상이므로 별도 질의가 필요하다. 좌표는 항상 건네받은 fresh 트리에서 다시 읽어 낡은 좌표 가드가 경로 홉에도 적용된다.

### Changed — 탐험이 화면 하나에서 멈추지 않는다
- `device_wda.sh explore` 가 현재 화면의 frontier 를 소진한 뒤 **스스로 미탐험 화면으로 이동해** 계속한다. 종료 조건은 전역 frontier 소진·max steps·`CLONE_EXPLORE_MAX_ROUTE`(기본 12) 연속 경로 홉이며, "다른 화면으로 이동한 뒤 다시 실행하라"는 사람 지시로 끝나지 않는다.
- `clone_flow_codegen.py` 가 producer 가 실제로 쓰는 `statekey`/`from_statekey`/`to_statekey` 를 먼저 읽는다. 그 전에는 underscored alias 만 읽어 조용히 coarse `node` 로 되돌아갔고, 한 화면의 두 상호작용 상태(포커스된 검색창과 아닌 것)가 한 뷰로 뭉개졌다.
- `autobot-clone-app` 스킬의 사람 게이트를 자율 기본값으로 바꿨다 — 재현 대상은 증거가 있는 전 화면, 화면 매핑은 자동 생성본을 그대로 사용, flow 맵은 열되 막지 않음, 데이터 필요 화면은 표시하고 계속(`CLONE_ASK_FOR_DATA=1` 로만 질문).

### Fixed — 라이브 앱(Threads)에서 탐험이 0 step 으로 죽던 문제
- **탭 freshness 가드가 `sig`(라벨 집합 해시) 대신 `statekey` 를 본다.** Threads 실기기 실행에서 탐험이 매번 0 step 으로 끝났다 — 캡처와 탭 사이에 좋아요 수가 `226명` → `227명` 으로 오르는 것만으로 라벨 집합이 달라져 후보 43개가 전부 거절됐다. 화면은 움직이지 않았고, 같은 churn 에서 `statekey`/`nodekey` 는 그대로였다. 라이브 피드에서 옛 가드는 통과가 **구조적으로 불가능**했다.
- **provenance 검사를 낡은 캡처가 아니라 라이브 트리에 돌린다.** 더 느슨한 게 아니라 더 엄격하다 — withheld(상태 변경) 판정도 이제 지금 화면 기준이다. 실측상 후보 좌표 43개 중 41개가 콘텐츠 churn 을 견디므로 재계획 비용도 작다.
- **가드 거절은 `exit 2`(재시도 가능), 탭 이후 실패는 `exit 1`(치명적).** `explore` 는 2 를 받으면 재캡처해서 다시 고르고(연속 `CLONE_EXPLORE_MAX_STALE`, 기본 5회), 실기기를 두 번 누르지 않는다. 그 전에는 화면이 한 번 움직이면 실행 전체가 끝났다.
- **settle 판정도 `sig` → `statekey`.** 카운터가 한 번 튀는 것을 "화면 전환"으로 읽으면 전이 중간 트리가 측정용 증거로 남는다.
- **WDA 실패 진단을 버리지 않는다.** `showXcodeLog` 를 항상 켜고(로그는 콘솔이 아니라 파일), code 65 면 마지막 xcodebuild 호출 구간의 오류 줄을 그대로 출력한다. `CLONE_WDA_DEBUG=1` 로 재실행하라는 안내는 실기기 세션 하나를 더 태우는 비용이었다.
- `clone_run.sh` 의 bundle ID 해석이 `devicectl --search` 에 의존하지 않는다 — 이 toolchain 에서 검색 필터가 무력해(존재하지 않는 검색어에도 설치 앱 77개 반환) 이름이 정확히 일치하지 않는 앱은 "77개 매치"로 실패했다.

### Fixed — `verify` 가 검사하지 못한 화면을 조용히 빼지 않는다
- `views.json` 이 매핑했지만 `screens/` 에 측정이 없는 state 는 비교할 대상이 없어 루프가 아예 보지 못했고, 요약은 **검사한 것만** 셌다. 실측 2026-08-22: 31개 state 를 매핑해 놓고 "28 of 28 screen(s) failed" 로 보고했다 — 31개짜리 앱을 전부 봤다는 말로 읽힌다.
- 이제 그런 state 를 이름으로 내고(`... have no measurement ...`) 실패로 센다. 요약도 `N problem(s) ... across M mapped screen(s) — C checked, U unverifiable` 로 바뀌어 분모가 매핑 수와 맞는다.

### Added — 실행이 자신이 무엇을 눌렀는지 감사한다
- `device_flow.py audit <flow.jsonl>` — 로그에 기록된 탭·스와이프를 **지금의 가드**로 다시 판정하고, 상태 변경 대상이 하나라도 있으면 non-zero. `clone_run.sh observe` 가 매 실행 끝에 자동으로 돌린다.
- 가드의 구멍은 열려 있는 동안 보이지 않는다 — 탭이 그냥 성공하고 로그에도 평범한 탭으로 남는다. 2026-08-22 사고에서는 두 번의 실행이 지나간 뒤 로그를 손으로 감사하고 나서야 드러났다. 이제 실행 자체가 그 감사를 하고, 가드를 고친 뒤 옛 로그를 다시 판정하는 데에도 쓴다.

### Fixed — 상태 변경 가드가 실제 기기 라벨에서 뚫려 있었다 (SECURITY/SAFETY)
- **탐험이 사용자의 실제 계정으로 남의 게시물에 좋아요·공유를 눌렀다** (실측 2026-08-22, Threads). `STATE_CHANGING` 의 모든 패턴이 라벨 **끝**에 앵커돼 있는데(`(?:^|\s)좋아요…$`), 실제 접근성 라벨은 `좋아요. 226명이 이 게시물을 좋아합니다.` 처럼 **액션 이름 + 설명문** 형태다. 앵커가 맞지 않아 좋아요·리포스트·공유가 전부 일반 탭 후보로 분류됐다.
- 끝 앵커는 `팔로우 추천`(화면)이 `팔로우`(액션)로 읽히는 것을 막는 장치라 없앨 수 없다. 그래서 전체 라벨과 **첫 절**(첫 문장 종결 부호 앞)을 함께 검사한다 — 정밀도를 유지하면서 구멍을 닫는다. `답글 달기…`(작성 화면으로 이동)·`좋아요한 글`(프로필 탭)·`게시 옵션` 은 그대로 탐험 대상이다.
- `sharing` 카테고리를 추가했다 — 시스템 공유 시트는 앱 밖으로 콘텐츠를 보낼 수 있고 대상 앱을 전면에서 밀어낸다.
- 실측 효과: 같은 로그에서 withheld 가 14 → 66. 기존 테스트는 기기가 절대 내보내지 않는 짧은 라벨(`좋아요`)만 검사해 이 실패를 통과시키고 있었다.

### Fixed — 앱을 벗어나는 탭이 그래프를 오염시키지 않는다
- **외부 앱 화면을 대상 앱의 state 로 기록하지 않는다.** Threads 의 `Instagram으로 전환` 을 탭하면 Appium `/source` 가 Instagram 을 기술하고, 그 화면이 `com.burbn.barcelona` 의 state 로 flow 로그에 들어갔다(실측 2026-08-22, `auto-0063`). 그대로 두면 측정되고 스펙이 쓰이고 SwiftUI 뷰로 매핑된다 — 복제 대상 앱의 화면인 것처럼. 이제 탭이 정착한 뒤 전면 앱을 확인하고, 다르면 이탈을 전이로만 기록한다.
- **이탈은 종착 엣지로 기록한다** — `to`/`to_statekey` 는 미해결, `changed=false`. 외부 앱의 statekey 를 남기면 `observed_edges` 가 화면 증거가 없는 state 로 라우팅하고 `capture_gaps` 가 영영 쓰이지 않을 도착 캡처를 요구한다(닫을 수 없는 공백).
- **이탈은 치명적이지 않다** — `explore` 가 대상 앱을 다시 전면으로 올리고(`mobile: activateApp`) 새 캡처에서 계속한다. 링크 하나 때문에 앱의 나머지를 포기하지 않는다. 연속 한도는 `CLONE_EXPLORE_MAX_LEAVE_APP`(기본 8).
- 성공한 스크롤은 `route_hops` 를 되돌린다 — 새 후보를 연 스크롤은 라우팅 캡이 막으려는 ping-pong 이 아니라 진전이다.

### Fixed — 탐험이 첫 화면분에서 끝나지 않는다 (목록을 스크롤한다)
- `explore` 가 탭할 후보가 하나도 없을 때 스크롤 후 재캡처하고 다시 고른다. `next-tap` 은 이미 "이 캡처 소진"과 "전역 소진"을 구분해 말하고 있었는데, `explore` 가 둘 다 종료로 처리했다. Threads 실측(2026-08-22): 타깃 235개 중 34개만 탐험됐고 나머지는 **애초에 어떤 캡처에도 없었다** — 첫 화면분 아래는 본 적이 없었다.
- 스크롤은 원래 멈췄을 자리에서만 일어나므로 기존 탐험 순서를 바꾸지 않는다. 화면당 `CLONE_EXPLORE_MAX_SCROLL`(기본 6)회, 그리고 화면이 실제로 움직이지 않으면 그 자리가 끝이다 — 무한 피드가 실행을 독점하지 못한다.
- 움직이지 않은 스크롤도 `swipe`+`screen` 으로 남긴다("봤는데 더 없었다"도 결과이며, `changed=false` 이므로 커버리지 공백이 되지 않는다).
- `_elements_moved` 는 0 이동 / 1 미이동 / **2 판정 불가**를 구분한다. 2 를 1 로 접으면 읽을 수 없는 트리가 조용히 "아무 일도 없었다"가 된다 — 판정 불가는 `changed=true` 로 기록해 `stats` 에 드러나게 한다(공백은 보이지만 잃은 화면은 보이지 않는다).

### Fixed — `verify` 가 실제로 검사하게 만든다 (Threads 실기기 회차에서 드러난 것)
- **구조 diff 의 라벨 매칭이 한 번도 동작한 적이 없었다.** `axe describe-ui` 는 `AXLabel` 을 쓰는데 `clone_structural_diff.py` 는 `label` 을 읽었다 — 라벨 매칭은 항상 빈 결과였고, 모든 요소가 8pt 프레임 매칭으로 떨어져 safe-area 만큼(14pt) 밀린 요소가 전부 `missing` 으로 보고됐다. WARN(위치 이탈) 경로는 죽은 코드였다. 이는 Step 6-5 의 우선순위 표를 뒤집는다: 간격 문제가 최상위 "요소 누락" 으로 도착한다. 테스트 픽스처가 producer 가 만들지 않는 `label` 키를 지어내 이 실패를 가리고 있었다.
- **시뮬레이터를 이름이 아니라 기기 종류로 고른다.** `auto` 가 시뮬레이터 *이름* 이 `iPhone 12 mini` 와 같기를 요구했는데, 스킬 문서가 안내하는 생성 명령(`simctl create clone-probe ...iPhone-12-mini`)은 그런 이름을 만들지 않는다. 실기기와 같은 기종의 시뮬레이터 3개가 있는데도 "no available simulator" 로 끝났다. 이제 `deviceTypeIdentifier` 로 맞추고(이름 일치도 계속 인정), 실패 메시지가 만들 명령을 그대로 준다.
- **환경 실패를 화면 수만큼 반복하지 않는다.** `verify` 가 렌더 루프 전에 시뮬레이터를 한 번 확정한다. 그 전에는 같은 환경 실패가 20번 "did not render — fix the compiler diagnostics above" 로 나왔다 — 있지도 않은 원인을 지목하면서. `CLONE_RENDER_SIMULATOR` 로 고정할 수 있다.
- **픽셀 지표를 크기 차이로 버리지 않는다.** 실기기 캡처는 1124x2436(WDA 렌더 버퍼), 시뮬레이터는 1080x2340(네이티브 패널)이다. 같은 375x812 pt 인데도 `device_compare.py` 가 비교를 거부해 이 기기에서는 정량 지표와 heatmap 이 **한 번도 실행되지 않았다**(실측 60/60 skip). 종횡비가 같으면 리샘플해서 비교하고, 진짜 종횡비 불일치 경고는 렌더 원본 크기로 판정한다.
- **AXe 트리 실패를 통과로 세지 않는다.** 첫 렌더는 시뮬레이터 부팅과 경합해 `axe describe-ui` 가 빈 결과를 낸다. 그때 구조 diff 가 조용히 건너뛰어져 그 화면이 "통과" 로 보고됐다 — 검사되지 않은 화면인데. 이제 한 번 재시도하고, 원인을 출력하고, AXe 가 설치돼 있는데도 트리가 없으면 그 화면을 실패로 센다.

### Fixed — swipe 의 `changed` 가 스크롤을 의미하게 한다
- settle 을 "처음으로 달라진 순간" 이 아니라 **연속 두 번 같은 화면** 으로 바꿨다. 전자는 600ms 스크롤 애니메이션 0.3초 지점의 트리를 도착 증거로 남기고, Step 3 가 그 반쯤 스크롤된 프레임을 기하로 측정한다.
- `changed` 를 라벨 집합이 아니라 **요소가 움직였는지** 로 판정한다. 라벨 집합은 두 방향 모두 틀린다: 라이브 피드에서는 스와이프 없이도 바뀌고(카운터), 짧은 목록을 스크롤하면 바뀌지 않는다. `changed` 는 `device_flow.py` 가 도착 캡처를 요구하는 조건이라 오탐은 닫을 수 없는 커버리지 공백이 되고, 미탐은 화면을 잃는다.

### Added — `/autobot:ssot` 설문 모드 (한 질문마다 멈추지 않는다)
- `scripts/interview_form.py` — 질문 spec(JSON)을 `127.0.0.1:8766` 폼으로 서빙하고 답변을 JSON으로 회수하는 단일 파일 stdlib 서버. 연관 없는 질문을 한 페이지에 모아 받고, 빈칸은 `skipped`(= 불필요한 질문)로 기록한다. 질문 타입 `text`·`textarea`·`choice`·`multi` + `hint`/`other`/`confirm`.
- **중립 설문**: 구글 설문지풍의 무채색 카드 레이아웃(라이트 고정 — 시스템 테마를 따라가지 않는다)이며, 기본 선택·미리 입력·"추천" 표시를 **지원하지 않는다** (기본값은 답을 유도한다). 스캔으로 아는 값은 `hint` 의 "현재 파악: …" 로만 보여준다.
- **대화 먼저, 설문은 그 다음**: 시작하자마자 띄우지 않는다. 대화로 기본 문답(제품 한 줄 정의 · 누구의 어떤 문제)을 먼저 해 맥락을 잡고, 물을 게 4개 이상이거나 2개 이상 주제를 가로지를 때 설문지로 전환한다 (그 이하는 대화, 되돌리기 어려운 갈림길은 AskUserQuestion).
- "다음" = 답변을 `ssot/*.md`에 즉시 기록한 뒤 더 깊은 다음 장을 만들어 같은 포트로 재기동(대기 페이지가 `round` 증가를 감지해 자동 전환), "완료" = 질문 종료 후 R6 확정으로.
- 기동은 `nohup … & disown` 으로 세션에서 **분리**한다 — 추적 백그라운드 태스크로 띄우면 턴 종료 시 죽어 사용자가 제출을 못 한다(실측 3/3). 제출 대기는 Monitor 로 답변 파일을 감시하고, 서버가 죽어도 열려 있던 폼 탭은 같은 포트로 재기동하면 그대로 제출된다.
- `autobot-ssot` 철칙 개정: 설문이 질문 회수 기본, AskUserQuestion은 되돌리기 어려운 갈림길 전용, 무응답은 재질문 금지(`decisions.md`에 미결로 기록), 기록은 제출 즉시. 설문을 못 띄우면 대화 모드로 폴백.
