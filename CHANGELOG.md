...
- 파괴·상태변경 라벨 필터는 이 티어에서도 그대로 적용된다. 다만 **라벨 없는 컨트롤은 후보로 나오지도 검사되지도 않으므로**, 스킬이 탭 전에 스크린샷을 읽고 화면 종류(로그인·페이월·결제)를 LLM 이 판단하도록 계약을 명시했다.
- 행 포함 관계로 작은 후보를 버리는 필터는 이 티어에서 끈다. 라벨-리프끼리는 트리상 서로를 포함할 수 없어 겹침이 순수하게 기하학적인데, zeta 는 피드 카드가 반투명 탭바 뒤까지 뻗어 있어 이 필터가 탭바 네 개를 통째로 삼켰다.
- 폴백은 역할 어휘 **밖**의 요소에만 적용한다. 빈 상태 화면의 `AXStaticText`("입력 항목 없음")는 의도적으로 후보가 아니며, 그 규칙은 그대로 둔다. 화면을 덮는 배경 크기 리프도 제외한다 — 실측상 정당한 리프의 최대 크기는 프레임의 16%(피드 카드)다.
- **티어를 화면 단위 all-or-nothing 으로 두지 않는다.** "역할 티어가 하나라도 찾았으면 성공"으로 판정하면 운 좋은 요소 하나가 나머지를 가린다 — 검색 화면은 `AXTextField` 를 딱 하나 보고했고, 그것 때문에 태그 칩 15개가 안 보여 화면이 막다른 길이 됐다. 39개 화면 측정 결과 병합은 역할이 잘 나오는 화면에서 아무것도 늘리지 않고(채팅방 +0), 나머지에서 진짜 컨트롤을 되찾는다(`더보기`, 모델 카드 3장, 칩 15개). `WARN: role-blind screen` 은 이제 역할이 하나도 없는 화면에만 붙는다.
- 행 포함 드롭에서 라벨-리프를 제외했다. 그 드롭은 "큰 타깃 안의 무의미한 것"을 지우려는 것이고 무의미함을 `rank < 2` 로 판정하는데, `_action_rank` 는 1을 반환하지 않는다(0·2·3뿐 — Journal 화면의 죽은 텍스트는 rank 0 필터가 지운다). 즉 역할 후보에게 이 가지는 **도달 불가능한 죽은 조건**이었고, rank 1인 리프만 걸렸다. 포함 관계는 "안에 있다"와 "뒤로 지나간다"를 구분하지 못한다: 피드 카드가 반투명 탭바 뒤로 뻗어 `홈/대화/만들기/마이페이지` 를, 위로 스크롤된 카드가 스티키 칩 뒤로 뻗어 `추천/스토리챗/비주얼 노벨` 을 삼켰다.

### Added — `/autobot:copy` 가 화면 흐름도를 낸다
- 탐험 로그(`.autobot/clone/flow.jsonl`)는 무엇을 눌러 어디로 갔는지를 전부 갖고 있었지만 copy 스킬은 한 번도 읽지 않았다. Step 6 에서 `device_flow.py map` 으로 `.autobot/copy-analysis/flow-map.html` 을 만들고, Step 3 은 프론티어를 `todo`/`next-tap`/`stats` 에 물어보도록 배선했다. 로그 초기화 정책(같은 앱은 이어서, 다른 앱이면 시작 전 삭제)도 명시했다.
- 흐름도가 **어디를 눌렀는지 스크린샷 위에 보여준다.** 전이 선이 카드 하단이 아니라 실제 탭 좌표에서 출발하고, 아직 안 눌러본 후보는 그 자리에 빈 점으로 찍힌다 — 미탐이 몇 개인지가 아니라 어디인지가 보인다.

### Fixed — 흐름도가 스크린샷을 자르고, 남의 화면에 탭을 그리고, 마커를 숨겼다
- **마커가 카드 밑에 깔렸다.** SVG 가 DOM 에서 먼저 나오고 z-index 도 없는데 카드는 불투명하다 — 좌표는 정확한데 완전히 안 보였다. 카드 뒤(연결 곡선)와 카드 위(탭 마커·리더선) 두 레이어로 나눴고, 그 순서를 테스트로 못박았다.
- **스크린샷이 잘렸다.** 카드를 200x232 로 고정하고 `object-fit: contain` 에 기댔더니 렌더는 이미지를 자르고, 마커는 있지도 않은 레터박스 오프셋 기준으로 놓였다. 이제 카드를 캡처 자신의 프레임 비율로 만든다 — 잘림도 레터박스도 없고, 탭 (x,y) 는 (x/fw, y/fh) 로 보정항 없이 떨어진다.
- **서로 다른 화면이 한 카드로 합쳐졌다.** `statekey`/`nodekey` 는 트리의 *모양*을 해시하는데 커스텀 렌더러 앱엔 모양이 없다 — zeta 는 모든 화면이 coarse node `da39a3ee5e6b`(= sha1(""))였다. 홈·만들기·랭킹·콘테스트가 한 카드가 됐고, 한 화면에서 한 탭이 다른 화면 스크린샷 위에 그려졌다. 맵은 이제 `sig`(라벨 집합) + `statekey` 복합키로 화면을 가른다. 전이는 로그의 시간 순서로 잇는다 — 탭 이벤트가 기록하는 statekey 가 바로 합쳐지는 그 정체성이기 때문이다.
- **모든 카드가 한 줄에 몰렸다.** 깊이 계산이 첫 캡처만 시드로 썼는데 그 화면엔 나가는 엣지가 없어 전파가 시작조차 못 했다. 유입 엣지가 없는 모든 화면을 루트로 삼는다.
- **선이 화면을 뒤덮었다.** 미탐 196개를 각각 카드로 그리면서 점선이 캔버스를 뒤덮었고, 그러면서도 "몇 개"밖에 말하지 못했다. 이제 스크린샷 위의 점이고, 선 굵기와 불투명도도 낮췄다.
- 배너 숫자가 카드가 아니라 statekey 기준이라 `행동 클래스 -47/219` 같은 음수를 찍었다. 맵이 실제로 그린 것만 센다.

### Added — `swipefrac`: 스와이프 좌표에서 단위를 없앤다
- 탭 좌표는 언제나 `candidates` 가 포인트로 주지만 **스와이프만 에이전트가 좌표를 지어내야 한다**. 그리고 그 좌표는 보통 스크린샷을 보고 정하는데, 스크린샷은 디바이스 픽셀(3배)이고 이미지 뷰어가 표시용으로 한 번 더 줄이기도 해서 **두 번 변환**해야 포인트가 된다. 실측 2026-08-27: 시트 핸들 307pt 를 240pt 로 계산했고 240pt 는 시트 위 스크림이라 닫기 시도 두 번이 아무 일도 하지 않았다 — 드래그 한 번이면 이어졌을 화면에서 탐험을 접었다.
- `device_wda.sh swipefrac <sid> <fx1> <fy1> <fx2> <fy2> [tree]` 를 추가했다. 앱 프레임의 비율(0..1)이라 변환할 단위가 없다. 프레임 원점 오프셋을 반영하고, 0..1 밖이면 거부하며, 변환 결과를 `INFO: swipefrac ... = x1 y1 x2 y2` 로 찍어 검산할 수 있게 한다.
- `screen` 이 캡처마다 `INFO: bounds <w>x<h> pt` 와 `INFO: scale <n>x` 를 출력한다. 추측해야 했던 값을 그냥 말해준다.
- 스킬에 규칙을 명시했다: 스와이프 좌표는 `swipefrac` 이거나 트리에서 읽은 포인트여야 하며, 스크린샷 픽셀에서 환산하지 않는다.

### Added — `back`: 라벨 없는 뒤로가기 때문에 막다른 화면에 갇히던 것
- 상세 화면의 뒤로가기가 라벨 없는 chevron 이면 후보가 하나도 없고 탭바도 없다. 게다가 이 앱은 iOS 인터랙티브 pop 제스처도 무시한다 — 실측 2026-08-27, 한 세션에서 **세 번**(검색·검색 결과·크리에이터 프로필) 에이전트가 스스로 걸어 들어간 화면에서 갇혔다.
- `device_a11y.py back <tree>` 가 네비게이션 바의 리딩 컨트롤을 기하학으로 찾고, `device_wda.sh back <sid>` 가 그것만 탭한다. 라벨 없는 요소를 대상으로 하는 **유일한** 예외이며, 그 슬롯의 의미는 앱이 아니라 플랫폼 관례가 정한다(뒤로/닫기/취소 — 결제도 삭제도 아니다).
- 후보 목록에는 넣지 않는다. 명시적 명령이라 사고로 눌리지 않고, "라벨 없는 것은 후보가 아니다"라는 규칙은 그대로다. 탭 루트(홈·랭킹)에서는 아무것도 내지 않는다.

### Fixed — 파괴 필터가 인앱 재화 충전과 "프로필 공유"를 통과시켰다
- `충전`(및 `top up`/`recharge`)이 구매 어휘에 없었다. zeta 마이페이지는 피스 잔액 옆 주 CTA 라벨이 딱 `충전` 이고, 삭제·구매·결제·구독 어느 것에도 걸리지 않아 후보로 나왔다(실측 2026-08-27). 이름만 다른 결제라 파괴 목록에 넣었다.
- 공유 패턴이 `^공유$` 로 앵커돼 있어 실제 버튼을 하나도 못 잡았다 — 한국어는 명사가 앞에 오므로 라벨이 `프로필 공유` 다. 시스템 공유 시트는 대상 앱을 포그라운드에서 밀어내므로 막아야 한다. 앞 명사를 허용하되 뒤 앵커는 유지해서 `공유 설정`·`공유된 항목`(화면)은 그대로 네비게이션으로 남는다.
- 가격을 적어 둔 컨트롤을 막는 `spend` 분류를 추가했다. zeta 채팅방의 일러스트 버튼은 라벨이 `스냅샷 15피스` 뿐이라 구매 어휘가 하나도 없는데 탭하는 순간 잔액이 깎인다. 신호는 가격 자체다 — 숫자 + 통화/인앱 재화(피스·코인·크레딧·젬·₩·$…). 잔액 표시도 함께 걸리지만 그건 후보 하나를 잃을 뿐 손해가 없다.
- `등록`·`임시저장`(한국어 저작도구의 발행·초안 저장)과 `대화방 나가기`(대화를 버린다)도 막았다. 셋 다 영어 어휘에 대응어가 없어 통과하고 있었고, 탐험이 사용자 계정에 데이터를 남기거나 지울 수 있었다.
- 여섯 구멍 모두 role-blind 티어와 무관하게 원래 있던 것이다 — 역할을 보고하는 앱에서도 같은 라벨이면 통과했다.
