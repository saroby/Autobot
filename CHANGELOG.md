...
- **ScrollView 가 없었다.** 122개 캡처 중 109개에 스크롤 컨테이너가 있는데 재현본은 고정 스택이었다. 접근성 트리가 크롬과 콘텐츠를 이미 분리해 주므로(Threads 상단바·탭바는 피드의 자손이 아니다) 그대로 나눠 렌더한다 — 콘텐츠는 `ScrollView` 안에 컨테이너 기준 좌표로, 크롬은 밖에 고정. 탭 타깃도 각자의 레이어에. 스와이프로 움직이는 것을 시뮬레이터에서 실측했다.
