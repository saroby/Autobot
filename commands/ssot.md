...
2. **R1–R5 인터뷰** — 존재 이유 · 기능 세트 · 도메인 · 원칙 · 성격. **대화로 기본 문답(한 줄 정의 · 누구의 어떤 문제)을 먼저** 하고, 물을 게 여러 주제로 쌓이면 **설문지**로 전환: 브라우저 폼 한 장에 모아 받고("다음" = 답변 기반 심화 질문지, "완료" = 인터뷰 종료), 빈칸은 불필요한 질문으로 보고 다시 묻지 않는다. 설문을 못 띄우면 대화 모드로 폴백.
3. **R6 확정** — 청사진 스냅샷 승인
