...
- **한방 실행** — 기본 경로는 세 명령이고, 모델이 손으로 쓰는 것은 화면 스펙과 **기계 초안의 손질**이다.
  ...
  scripts/clone_run.sh codegen                             # 선택: flow에서 생성만 복구, 기기 불필요
  scripts/clone_run.sh install [RootView]                  # 선택: 검증한 clone을 실기기에 설치
  ```
...
- **결과물** — `.autobot/clone/flow-map.html`(화면 지도) + `screens/*.{json,md}`(기계 소유 측정 증거) + `assets/manifest.json`(연구용 자산 provenance) + `Sources/*.swift`(화면·전이 router) + `compare/*.png`(원본 대조). 선택적인 `/autobot:mvp` 인수인계는 `reverse-brief.md`와 `specs/<ViewName>.md`에 사람이 작성한다.
...
4. **대조 이미지 없이 완료 선언 금지, 그리고 대조가 통과해야 한다** — 재현본을 시뮬레이터에서 렌더해 원본과 나란히 붙이고, 시각 mismatch 가 `--max-mismatch`(기본 `0.30`, `CLONE_MAX_MISMATCH`) 안에 들어와야 완료다. 이미지를 만든 것 자체는 완료가 아니다. 대조는 수렴 루프다: 요소 누락·구조 차이를 먼저 없애고, 남은 차이가 스펙에 선언된 재현 불가 항목뿐일 때 끝난다.
